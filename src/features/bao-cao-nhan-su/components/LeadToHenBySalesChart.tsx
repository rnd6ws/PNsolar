            ...
            <div className="bg-card border border-border rounded-xl shadow-lg px-4 py-3 min-w-[200px]">
                ...
                <div className="space-y-1 text-sm">
                    <div className="flex justify-between gap-4">
                        <span className="text-muted-foreground">Tổng Lead</span>
                        <span className="font-medium">{d?.totalLead}</span>
                    </div>
                    <div className="flex justify-between gap-4">
                        <span className="text-muted-foreground">Hẹn đã gặp</span>
                        <span className="font-medium text-emerald-500">{d?.totalHen}</span>
                    </div>
                    <div className="flex justify-between gap-4 border-t border-border pt-1 mt-1">
                        <span className="text-muted-foreground">Tỷ lệ</span>
                        <span className="font-bold text-indigo-500">{d?.rate}%</span>
                    </div>
                </div>
            </div>
        );
...
// Màu gradient theo tỷ lệ
function getRateColor(rate: number): string {
    if (rate >= 30) return "#10b981"; // emerald
    if (rate >= 15) return "#6366f1"; // indigo
    if (rate >= 5) return "#f59e0b";  // amber
    return "#ef4444";                  // red
}

export default function LeadToHenBySalesChart({ data }: Props) {
    ...
    if (!mounted) return <div className="w-full h-[360px] animate-pulse bg-muted rounded-2xl" />;

    const chartData = data.filter(d => d.totalLead > 0 || d.totalHen > 0);
            ...
            <div className="w-full bg-card border border-border rounded-2xl shadow-sm p-5 animate-in fade-in duration-300">
                <div className="flex items-center gap-2 mb-1">
                    ...
                    <h3 className="font-bold text-base md:text-lg text-foreground">
                        1.2a · % Lead → Hẹn theo Sales (Telesales)
                    </h3>
                </div>
        ...
        <div className="w-full bg-card border border-border rounded-2xl shadow-sm p-5 animate-in fade-in duration-300">
            <div className="flex flex-col mb-4">
                <div className="flex items-center gap-2 mb-1">
                    ...
                    <h3 className="font-bold text-base md:text-lg text-foreground">
                        1.2a · % Lead → Hẹn (Đã gặp) theo Sales
                    </h3>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-4">
                {chartData.slice(0, 3).map((d, i) => (
                    <div
                        ...
                        style={{
                            background: `${getRateColor(d.rate)}12`,
                            border: `1px solid ${getRateColor(d.rate)}30`,
                        }}
                    >
                        <span className="text-xs text-muted-foreground truncate">{i === 0 ? "🥇" : i === 1 ? "🥈" : "🥉"} {d.sales}</span>
                        <span className="text-lg font-bold" style={{ color: getRateColor(d.rate) }}>
                            {d.rate}%
                        </span>
                        <span className="text-xs text-muted-foreground">{d.totalHen}/{d.totalLead} hẹn</span>
                    </div>
                ))}
            </div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                        ...
                        <Bar dataKey="rate" name="% Hẹn gặp" radius={[0, 6, 6, 0]} barSize={20} isAnimationActive={true}>
                            {chartData.map((d, i) => (
                                <Cell key={`cell-${i}`} fill={getRateColor(d.rate)} />
                            ))}
                            <LabelList
                                dataKey="rate"
                                position="right"
