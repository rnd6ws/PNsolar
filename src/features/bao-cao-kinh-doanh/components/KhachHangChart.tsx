...
export default function KhachHangChart({ data }: Props) {
    ...
    if (!data || data.length === 0) {
        return (
            <div className="w-full h-[340px] flex flex-col items-center justify-center bg-card border border-border rounded-2xl shadow-sm gap-3">
                <Users className="w-10 h-10 text-muted-foreground/40" />
                <p className="text-sm text-muted-foreground">Không có dữ liệu khách hàng</p>
            </div>
        );
    }
        ...
        <div className="w-full bg-card border border-border rounded-2xl shadow-sm p-5 animate-in fade-in duration-300">
            <div className="flex items-center gap-2 mb-4">
                ...
                <span className="ml-auto text-xs text-muted-foreground bg-muted px-2 py-1 rounded-full">
                    {data.reduce((s, d) => s + d.count, 0)} tổng
                </span>
            </div>
            <div className="w-full h-[260px]">
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={data} margin={{ top: 8, right: 10, left: -10, bottom: 5 }}>
                        <CartesianGrid
                            strokeDasharray="3 3"
                            vertical={false}
                        ...
                        <Bar dataKey="count" name="Khách hàng" radius={[6, 6, 0, 0]} maxBarSize={44}>
