...
import {
    Settings2, Download, Calendar, Grid3x3, Users, FileText,
    LayoutList, LayoutGrid, X, ChevronDown,
} from "lucide-react";
import { cn } from "@/lib/utils";
import SearchInput from "@/components/SearchInput";
import ColumnToggleButton, { type ColumnKey, DEFAULT_COLUMNS } from "./ColumnToggleButton";
import DeNghiTTList from "./DeNghiTTList";
import {
...
export default function DeNghiTTPageClient({ data }: { data: any[] }) {
    ...
    const clearDateFilter = () => {
        const params = new URLSearchParams(searchParams.toString());
        params.delete("NGAY_TU");
        params.delete("NGAY_DEN");
        params.delete("page");
        router.replace(`${pathname}?${params.toString()}`);
    };
    ...
    // ─── FilterRow (dùng chung Desktop + Mobile) ────────────────
    const FilterRow = (
        <>
            {/* Ngày đề nghị — dropdown date range */}
            <DropdownMenu>
                <DropdownMenuTrigger asChild>
                    ...
                    >
                        ...
                        <span>{hasDateFilter ? `${ngayTu || "..."} → ${ngayDen || "..."}` : "Ngày ĐN"}</span>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <div>
                                ...
                                <input
                                    type="date"
                                    value={ngayTu}
                                    onChange={e => updateParams({ NGAY_TU: e.target.value })}
                            ...
                            <div>
                                ...
                                <input
                                    type="date"
                                    value={ngayDen}
                                    onChange={e => updateParams({ NGAY_DEN: e.target.value })}
                                    className="w-full h-8 px-2 text-xs bg-background border border-input rounded-md focus:outline-none focus:ring-2 focus:ring-ring"
                                />
                            </div>
                        </div>
            ...
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-48 rounded-xl font-medium">
                    <DropdownMenuItem
                        onClick={() => setGroupBy("MA_KH")}
                        className={cn("py-2.5", groupBy === "MA_KH" && "bg-primary/10 text-primary")}
                    >
                        <Users className="w-4 h-4 mr-2" /> Khách hàng
                    </DropdownMenuItem>
                    <DropdownMenuItem
                        onClick={() => setGroupBy("SO_HD")}
                        className={cn("py-2.5", groupBy === "SO_HD" && "bg-primary/10 text-primary")}
                    >
    ...
    return (
        <>
            ...
            <div className="p-5 flex flex-col gap-4 text-sm font-medium border-b border-primary/10 bg-linear-to-b from-primary/3 to-primary/8">
                <div className="flex items-center justify-between gap-3 w-full">
                    ...
                    <div className="flex-1 w-full lg:max-w-[360px]">
                        <SearchInput placeholder="Tìm mã đề nghị, khách hàng, hợp đồng..." />
                    </div>
                ...
                {showFilters && (
                    <div className="flex lg:hidden flex-col gap-3 w-full bg-muted/30 p-4 rounded-xl border border-border animate-in slide-in-from-top-2 fade-in duration-200">
                        <div className="flex flex-col gap-3">
                            <div className="grid grid-cols-2 gap-2">
                                <div>
                                    <label className="text-xs text-muted-foreground mb-1 block">Ngày ĐN từ</label>
                                ...
                                <div>
                                    <label className="text-xs text-muted-foreground mb-1 block">Ngày ĐN đến</label>
            ...
            <div className="p-0">
                <DeNghiTTList data={data} visibleColumns={visibleColumns} viewMode={viewMode} groupBy={groupBy} />
            </div>
        </>
    );
}
