    ...
    return (
        <>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
                <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-5xl max-h-[92vh] flex flex-col animate-in zoom-in-95 duration-200">
                    {/* Header */}
                    ...
                    <div className="flex-1 overflow-y-auto p-5 space-y-5">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                                ...
                                <div>
                                    ...
                                    <select className={selectClass} value={selNCC} onChange={e => setSelNCC(e.target.value)}>
                                        ...
                                        {nccOptions.map(n => (
                                            <option key={n.ID} value={n.MA_NCC}>{n.TEN_NCC}</option>
                                        ))}
                                    </select>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between pt-2 border-t border-border/50">
                                <div className="text-sm text-muted-foreground">
                                    {previewCount > 0 && selNCC ? (
                                        <span>
                                            Sẽ tạo: <strong className="text-foreground">{previewCount}</strong> hàng hóa × 1 NCC = <strong className="text-primary text-base">{previewCount}</strong> dòng
                                        </span>
                                    ) : (
                                ...
                                <button
                                    ...
                                    disabled={previewCount === 0 || !selNCC}
                                    className="inline-flex items-center gap-2 h-9 px-4 text-sm font-semibold bg-primary text-primary-foreground hover:bg-primary/90 rounded-lg transition-all active:scale-95 shadow-sm disabled:opacity-40 disabled:pointer-events-none"
                                >
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-3">
                                ...
                                {detailRows.length > 0 && (
                                    <button type="button" onClick={() => setDetailRows([])}
                                                    ...
                                                    <tr key={row.key} className="hover:bg-muted/20 transition-colors">
                                                        ...
                                                        <td className="px-3 py-2">
                                                            <input
                                                                ...
                                                                onChange={e => handleDonGiaChange(row.key, e.target.value)}
                                                            />
                                                        </td>
                                                        <td className="px-3 py-2">
