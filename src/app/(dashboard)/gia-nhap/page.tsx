import { getGiaNhapList, getNhomHHOptions, getPhanLoaiOptions, getDongHangOptions, getGoiGiaOptions, getNccOptions, getHangHoaOptionsForGiaNhap } from "@/features/gia-nhap/action";
...
export default async function GiaNhapPage({ searchParams }: { searchParams: any }) {
    ...
    const MA_GOI_GIA = params?.MA_GOI_GIA || "";
    const MA_NCC = params?.MA_NCC || "";
    const MA_HH = params?.MA_HH || "";

    const [result, nhomHHOpts, phanLoaiOpts, dongHangOpts, goiGiaOpts, nccOpts, hhOpts] = await Promise.all([
        getGiaNhapList({ page, limit: 15, query, MA_NHOM_HH, MA_PHAN_LOAI, MA_DONG_HANG, MA_GOI_GIA, MA_NCC, MA_HH }),
        getNhomHHOptions(),
        getPhanLoaiOptions(),
        getDongHangOptions(),
        getGoiGiaOptions(),
